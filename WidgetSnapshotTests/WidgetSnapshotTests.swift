...
private enum SnapshotOutput {
    ...
    private static func findRepoRoot(from start: URL) -> URL? {
        ...
        while FileManager.default.fileExists(atPath: current.path, isDirectory: &isDir), isDir.boolValue {
            let hasProject = FileManager.default.fileExists(atPath: current.appendingPathComponent("AgentUsage.xcodeproj").path)
            let hasWorkspace = FileManager.default.fileExists(atPath: current.appendingPathComponent("AgentUsage.xcworkspace").path)
